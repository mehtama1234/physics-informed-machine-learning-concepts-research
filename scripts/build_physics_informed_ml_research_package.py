...
MEATY_GOAL_REQUIREMENTS = [
    ...
    {"key": "why_care_before_terms", "label": "Why Care Before Terms", "proof_term": "Why Care Before Any Technical Term"},
    {"key": "workday_decision_rehearsal", "label": "Workday Decision Rehearsal", "proof_term": "Workday Decision Rehearsal"},
    ...
    {"key": "tiny_invented_case", "label": "Tiny Invented Case", "proof_term": "Tiny Invented Case Recipe"},
    ...
    {"key": "field_decision_story", "label": "Field Decision Story", "proof_term": "Everyday Field Decision Story"},
    ...
    {"key": "why_not_simpler_answer", "label": "Why Not Simpler Answer", "proof_term": "Why Not The Simpler Answer"},
    {"key": "self_diagnosis_check", "label": "Self-Diagnosis Check", "proof_term": "Self-Diagnosis Check"},
    ...
    {"key": "new_case_transfer_rehearsal", "label": "New Case Transfer Rehearsal", "proof_term": "New Case Transfer Rehearsal"},
    ...
    {"key": "one_page_mental_model", "label": "One-Page Mental Model", "proof_term": "One-Page Mental Model"},
        ...
        packets.append(
            {
                ...
                "proof_boundary": concept_proof_boundary(topic, derivation),
                "limits": [
...
def concept_proof_boundary(topic: dict[str, object], derivation: dict[str, object]) -> dict[str, str]:
    title = str(topic["title"])
    problem = str(topic["common_problem"])
    domain = str(topic["domain"])
    return {
        "transcript_can_support": f"The transcript can support that {title} is taught as a way to address this shortage: {problem}",
        "transcript_cannot_support": f"The transcript cannot prove that {title} will work on every new case in {domain}; that has to be tested on the target quantity.",
        "stronger_validation_needed": f"Run the idea on a changed case in {domain} and compare the predicted quantity with trusted measurements, trusted solves, or a known conservation check.",
        "first_overclaim_to_reject": f"Reject any sentence saying {title} is generally reliable before it survives this test: {topic['failure_boundary']}",
        "reviewer_action": f"Start from the selected source anchors, then ask whether the page's claim is no wider than this hidden need: {derivation['hidden']}",
    }
...
def topic_claim_chain_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    return f"""
<h2>Big Picture Claim Chain</h2>
<p>Read this row by row before trusting the method name. The concept earns its place only if each link carries the next one.</p>
<table>
  <tbody>
    <tr><th>Everyday Problem</th><td>{html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Domain Where It Matters</th><td>{html.escape(str(topic['domain']))}</td></tr>
    <tr><th>Decision Or Quantity At Stake</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Evidence In Hand</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Hidden Thing Needed</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Mathematical Move</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>What {html.escape(title)} Is Allowed To Claim</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>First Thing That Can Break The Claim</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
    <tr><th>Changed Case To Try First</th><td>{html.escape(str(derivation['test']))}</td></tr>
  </tbody>
</table>
"""
...
def topic_why_care_before_terms_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    ...
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Why Care Before Any Technical Term</h2>
<p>Before naming {html.escape(title)}, ask what would go wrong for a person doing real work. The person has this problem: {html.escape(str(topic['common_problem']))} The cost is not a bad quiz score. The cost is a decision made from evidence that does not yet reach the answer.</p>
<table>
  <tbody>
    <tr><th>Real Work At Risk</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Evidence The Person Has</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Answer The Person Still Needs</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Plain Move That Helps</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Topology Or Shape Reason</th><td>{html.escape(str(shape['use']))} The first shape check is: {html.escape(str(shape['check']))}</td></tr>
    <tr><th>Engineering Reason</th><td>{html.escape(str(engineering['use']))} Check: {html.escape(str(engineering['check']))}</td></tr>
    <tr><th>Materials Biology Or Chemistry Reason</th><td>{html.escape(str(materials['use']))} Check: {html.escape(str(materials['check']))}</td></tr>
    <tr><th>Climate Fluid Or Field Reason</th><td>{html.escape(str(fields['use']))} Check: {html.escape(str(fields['check']))}</td></tr>
    <tr><th>First Trust Limit</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
  </tbody>
</table>
<h3>Plain Importance Test</h3>
<p>The topic has been explained well only if the reader can say why it matters before using its name. For this page, that means saying how {html.escape(str(derivation['observed']))} can and cannot support {html.escape(str(derivation['hidden']))}, where shape or field structure enters, and what test would make the claim smaller.</p>
"""


def topic_workday_decision_rehearsal_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    case = topic_case_walkthrough(topic, derivation)
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    ...
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Workday Decision Rehearsal</h2>
<p>Use this as a slow spoken rehearsal. The learner should be able to tell the day as a chain of small choices, not as a definition of {html.escape(title)}.</p>
<table>
  <tbody>
    <tr><th>Morning Job</th><td>A person starts with this shortage: {html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>First Thing They Write Down</th><td>{html.escape(str(case['observed']))}</td></tr>
    <tr><th>Question They Cannot Answer Yet</th><td>{html.escape(str(case['hidden']))}</td></tr>
    <tr><th>Decision Waiting On The Answer</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Shape They Must Not Flatten</th><td>{html.escape(str(shape['use']))} Check: {html.escape(str(shape['check']))}</td></tr>
    <tr><th>Plain Move They Try Before Lunch</th><td>{html.escape(str(case['move']))}</td></tr>
    <tr><th>Engineering Version Of The Same Day</th><td>{html.escape(str(engineering['use']))}</td></tr>
    <tr><th>Lab Or Biology Version Of The Same Day</th><td>{html.escape(str(materials['use']))}</td></tr>
    <tr><th>Field Or Flow Version Of The Same Day</th><td>{html.escape(str(fields['use']))}</td></tr>
    <tr><th>End Of Day Claim</th><td>{html.escape(str(case['answer']))}</td></tr>
    <tr><th>Next Morning Recheck</th><td>{html.escape(str(case['rejection']))}</td></tr>
  </tbody>
</table>
<h3>Rehearsal Pass Test</h3>
<p>The rehearsal passes only if the learner can retell the workday in order: shortage, written evidence, missing answer, waiting decision, shape to protect, plain move, field transfer, careful claim, and next recheck.</p>
"""


def topic_sounds_right_filter_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    wrong = topic_wrong_use(topic)
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    ...
    return f"""
<h2>Sounds-Right Filter</h2>
<p>A weak explanation can sound confident while skipping the part that proves the claim. Use this filter before accepting any explanation of {html.escape(title)}.</p>
<table>
  <tbody>
    <tr><th>Sounds Right But Is Weak</th><td>{html.escape(wrong['mistake'])}</td></tr>
    <tr><th>Why Someone Might Believe It</th><td>{html.escape(wrong['why_tempting'])}</td></tr>
    <tr><th>Evidence It Must Name Instead</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Missing Answer It Must Name Instead</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Plain Move It Must Explain</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape It Must Protect</th><td>{html.escape(str(shape['use']))}</td></tr>
    <tr><th>Field Use It Must Ground</th><td>{html.escape(str(field_use['use']))}</td></tr>
    <tr><th>First Test It Must Survive</th><td>{html.escape(wrong['catch_test'])}</td></tr>
    <tr><th>What Breaks If It Fails</th><td>{html.escape(wrong['what_breaks'])}</td></tr>
  </tbody>
</table>
<h3>Filter Pass Test</h3>
<p>The explanation passes only if the learner can replace the weak sentence with a plain chain: evidence, missing answer, move, shape, field use, careful claim, and first test.</p>
"""


def topic_draw_before_math_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Draw Before Math</h2>
<p>Before writing a formula for {html.escape(title)}, make a small drawing. The drawing does not need to be pretty. It only needs to show what is known, what is missing, what relation must stay visible, and what changed case could break the claim.</p>
<table>
  <tbody>
    <tr><th>Box One: Problem</th><td>Write the everyday shortage: {html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Box Two: Evidence</th><td>Draw or list what is already seen: {html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Box Three: Missing Answer</th><td>Leave a blank for what is needed: {html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Arrow: Plain Move</th><td>Label the arrow with the move: {html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape Mark</th><td>Circle the relation that must stay visible: {html.escape(str(shape['use']))}</td></tr>
    <tr><th>Engineering Mark</th><td>Add one engineering use: {html.escape(str(engineering['use']))}</td></tr>
    <tr><th>Field Mark</th><td>Add one field or flow use: {html.escape(str(fields['use']))}</td></tr>
    <tr><th>Break Mark</th><td>Mark the first changed case to try: {html.escape(str(derivation['test']))}</td></tr>
    <tr><th>Claim Under The Drawing</th><td>Write only this kind of claim: {html.escape(str(derivation['meaning']))}</td></tr>
  </tbody>
</table>
<h3>Drawing Pass Test</h3>
<p>The drawing passes only if another person can point to the evidence, missing answer, move, shape relation, field use, changed case, and careful claim without hearing the topic name first.</p>
"""


def topic_start_here_gate_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    slug = str(topic["slug"])
    title = str(topic["title"])
    names = {str(item["slug"]): str(item["name"]) for item in CONCEPTS}
    dependency = next((item for item in CONCEPT_DEPENDENCIES if item["concept"] == slug), None)
    ...
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    read_first = ", ".join(names.get(dep, dep.replace("-", " ").title()) for dep in learn_first) if learn_first else "the everyday problem, evidence, missing answer, and changed case"
    next_open = ", ".join(names.get(dep, dep.replace("-", " ").title()) for dep in used_by) if used_by else "the field-use, source-limit, or rejection-test page that uses this claim"
    why_here = str(dependency["why"]) if dependency else f"{title} can be a starting page when the learner can already say what is observed, what answer is missing, and what changed case would reject the claim."
    return f"""
<h2>Start-Here Gate</h2>
<p>Use this gate before choosing {html.escape(title)} as the first topic to open. The topic is a good first stop only when the learner can name the shortage, evidence, missing answer, shape signal, and first test in ordinary words.</p>
<table>
  <tbody>
    <tr><th>Open This Topic When</th><td>The problem sounds like this: {html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Evidence Signal</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Missing-Answer Signal</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Shape Or Topology Signal</th><td>{html.escape(str(shape['use']))}</td></tr>
    <tr><th>Read This First If Needed</th><td>{html.escape(read_first)}</td></tr>
    <tr><th>Why This Page Is The Right Stop</th><td>{html.escape(why_here)}</td></tr>
    <tr><th>Next Page To Open After This</th><td>{html.escape(next_open)}</td></tr>
    <tr><th>Do Not Start Here When</th><td>{html.escape(str(topic['leaves_out']))} is the part the learner needs most.</td></tr>
    <tr><th>Start-Here Rejection Test</th><td>{html.escape(str(derivation['test']))}</td></tr>
  </tbody>
</table>
<h3>Gate Pass Test</h3>
<p>The gate passes only if the learner can say why this topic is the first stop, what should be read first when needed, what page comes next, and what changed case would prove the start was too broad.</p>
"""
...
def topic_oral_explanation_script_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    field_use = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Oral Explanation Script</h2>
<p>Use this as a spoken answer for {html.escape(title)}. The learner should be able to say it without looking at a formula or reading the topic name as the reason.</p>
<table>
  <tbody>
    <tr><th>Sentence One: Everyday Need</th><td>A person needs this idea because {html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Sentence Two: Evidence In Hand</th><td>What they can point to is {html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Sentence Three: Missing Answer</th><td>What they still need is {html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Sentence Four: Plain Move</th><td>The plain move is to {html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Sentence Five: Shape Or Topology</th><td>The shape issue is: {html.escape(str(shape['use']))}</td></tr>
    <tr><th>Sentence Six: Field Transfer</th><td>In a field or flow case, {html.escape(str(field_use['use']))}</td></tr>
    <tr><th>Sentence Seven: Careful Claim</th><td>The careful claim is: {html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>Sentence Eight: Belief Test</th><td>I would believe it only after this test: {html.escape(str(derivation['test']))}</td></tr>
  </tbody>
</table>
<h3>Spoken Pass Test</h3>
<p>The spoken answer passes only if another person can hear the need, evidence, missing answer, move, shape issue, field transfer, careful claim, and belief test in that order.</p>
"""


def topic_before_after_decision_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    ...
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Before-After Decision Check</h2>
<p>This check asks what changes after the learner understands {html.escape(title)}. If no decision changes, the explanation is still only a definition.</p>
<table>
  <tbody>
    <tr><th>Before Learning This</th><td>The learner may see {html.escape(str(derivation['observed']))} but not know how to use it for this missing answer: {html.escape(str(derivation['hidden']))}.</td></tr>
    <tr><th>After Learning This</th><td>The learner should try this move: {html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Decision They Can Now Make</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Shape Decision That Changes</th><td>{html.escape(str(shape['use']))} Check: {html.escape(str(shape['check']))}</td></tr>
    <tr><th>Engineering Decision That Changes</th><td>{html.escape(str(engineering['use']))} Check: {html.escape(str(engineering['check']))}</td></tr>
    <tr><th>Lab Or Biology Decision That Changes</th><td>{html.escape(str(materials['use']))} Check: {html.escape(str(materials['check']))}</td></tr>
    <tr><th>Field Decision That Changes</th><td>{html.escape(str(fields['use']))} Check: {html.escape(str(fields['check']))}</td></tr>
    <tr><th>Claim They Can Now Say</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>Decision They Must Still Refuse</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
    <tr><th>Changed Case That Decides</th><td>{html.escape(str(derivation['test']))}</td></tr>
  </tbody>
</table>
<h3>Before-After Pass Test</h3>
<p>The check passes only if the learner can say what decision was impossible before, what decision is possible after, what shape or field issue changes the decision, and what changed case still controls trust.</p>
"""
...
def topic_tiny_invented_case_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Tiny Invented Case Recipe</h2>
<p>The learner should be able to invent a tiny new case for {html.escape(title)} without copying the worked example. The case only has to be small enough to reveal the first-principles chain.</p>
<table>
  <tbody>
    <tr><th>Invent A Small Setting</th><td>Use a setting where this shortage appears: {html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Name One Thing Seen</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Name One Thing Missing</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Name The Small Move</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Add One Shape Detail</th><td>{html.escape(str(shape['use']))}</td></tr>
    <tr><th>Add One Engineering Detail</th><td>{html.escape(str(engineering['use']))}</td></tr>
    <tr><th>Add One Field Detail</th><td>{html.escape(str(fields['use']))}</td></tr>
    <tr><th>Say The Small Claim</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>Change One Thing To Test It</th><td>{html.escape(str(derivation['test']))}</td></tr>
  </tbody>
</table>
<h3>Invented Case Pass Test</h3>
<p>The invented case passes only if the learner can make up a small setting, name what is seen, name what is missing, make the move, include a shape or field detail, state a careful claim, and change one thing to test it.</p>
"""
...
def topic_misread_repair_drill_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    ...
    title = str(topic["title"])
    return f"""
<h2>Misread Repair Drill</h2>
<p>This drill catches the common moment when a reader knows the topic name but not the claim. Repair the misunderstanding by replacing the label-first sentence with an evidence-first sentence.</p>
<table>
  <tbody>
    <tr><th>Likely Misread</th><td>{html.escape(wrong['mistake'])}</td></tr>
    <tr><th>Why That Sounds Tempting</th><td>{html.escape(wrong['why_tempting'])}</td></tr>
    <tr><th>Plain Repair</th><td>{html.escape(title)} should be read as this move: {html.escape(str(derivation['move']))}. The move starts from {html.escape(str(derivation['observed']))} and tries to reach {html.escape(str(derivation['hidden']))}.</td></tr>
    <tr><th>Evidence To Name Before Trusting It</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Changed Case That Repairs The Overclaim</th><td>{html.escape(wrong['catch_test'])}</td></tr>
    <tr><th>One Sentence To Keep</th><td>Use {html.escape(title)} only when the evidence, hidden answer, domain use, shape issue, and rejection test can all be said without hiding behind the method name.</td></tr>
  </tbody>
</table>
"""
...
def write_topic_page(path: Path, topic: dict[str, object], reader_checks: list[dict[str, object]]) -> None:
    ...
    why_care_before_terms = topic_why_care_before_terms_html(topic, derivation)
    workday_decision_rehearsal = topic_workday_decision_rehearsal_html(topic, derivation)
    ...
    tiny_invented_case = topic_tiny_invented_case_html(topic, derivation)
    ...
    field_decision_story = topic_field_decision_story_html(topic, derivation)
    ...
    new_case_transfer_rehearsal = topic_new_case_transfer_rehearsal_html(topic, derivation)
    ...
    one_page_mental_model = topic_one_page_mental_model_html(topic, derivation)
    ...
    why_not_simpler_answer = topic_why_not_simpler_answer_html(topic, derivation)
    self_diagnosis_check = topic_self_diagnosis_check_html(topic, derivation)
...
{claim_chain}
{explanation_order}
{why_care_before_terms}
{workday_decision_rehearsal}
{sounds_right_filter}
{draw_before_math}
{start_here_gate}
...
def proof_boundary_html(boundary: dict[str, str]) -> str:
    return f"""
<h2>Transcript Evidence vs Scientific Proof</h2>
<p>This table separates a course-supported claim from proof that the method will work on a new scientific case.</p>
<table>
  <tbody>
    <tr><th>Transcript Can Support</th><td>{html.escape(str(boundary['transcript_can_support']))}</td></tr>
    <tr><th>Transcript Cannot Support</th><td>{html.escape(str(boundary['transcript_cannot_support']))}</td></tr>
    <tr><th>Stronger Validation Needed</th><td>{html.escape(str(boundary['stronger_validation_needed']))}</td></tr>
    <tr><th>First Overclaim To Reject</th><td>{html.escape(str(boundary['first_overclaim_to_reject']))}</td></tr>
    <tr><th>Reviewer Action</th><td>{html.escape(str(boundary['reviewer_action']))}</td></tr>
  </tbody>
</table>
"""
...
def topic_new_case_transfer_rehearsal_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    case = topic_case_walkthrough(topic, derivation)
    shape = next(row for row in topic_plain_applications(topic, derivation) if row["field"] == "Topology and shape")
    return f"""
<h2>New Case Transfer Rehearsal</h2>
<p>This rehearsal asks whether the idea survives a move from the worked case to a new case. The learner should keep the first-principles chain visible instead of copying the old answer.</p>
<table>
  <tbody>
    <tr><th>Original Case</th><td>{html.escape(str(case['setting']))}</td></tr>
    <tr><th>Original Evidence</th><td>{html.escape(str(case['observed']))}</td></tr>
    <tr><th>Original Missing Answer</th><td>{html.escape(str(case['hidden']))}</td></tr>
    <tr><th>New Case To Try</th><td>{html.escape(str(derivation['test']))}</td></tr>
    <tr><th>Evidence That Must Still Be Named</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Missing Answer In The New Case</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Move That Should Stay The Same</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape Or Boundary That May Change</th><td>{html.escape(str(shape['check']))}</td></tr>
    <tr><th>Claim Allowed After Transfer</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>First Transfer Rejection Signal</th><td>{html.escape(str(case['rejection']))}</td></tr>
  </tbody>
</table>
<h3>Transfer Rehearsal Pass Test</h3>
<p>The rehearsal passes only if the learner can move from the original case to the new case while naming the evidence, missing answer, same move, changed shape or boundary, allowed claim, and rejection signal.</p>
"""
...
def topic_one_page_mental_model_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    case = topic_case_walkthrough(topic, derivation)
    shape = next(row for row in topic_plain_applications(topic, derivation) if row["field"] == "Topology and shape")
    model = (
        f"{title} is a route from {str(derivation['observed'])} to {str(derivation['hidden'])}. "
        f"The route exists because {str(topic['common_problem'])}. "
        f"The move is {str(derivation['move'])}. "
        f"The shape issue is {str(shape['use'])}. "
        f"A concrete picture is {str(case['setting'])}. "
        f"The allowed claim is {str(derivation['meaning'])}. "
        f"The first test is {str(derivation['test'])}."
    )
    return f"""
<h2>One-Page Mental Model</h2>
<p>{html.escape(model)}</p>
<table>
  <tbody>
    <tr><th>Need</th><td>{html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Evidence</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Missing Answer</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Plain Move</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape To Keep</th><td>{html.escape(str(shape['use']))}</td></tr>
    <tr><th>Field Reason</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Concrete Picture</th><td>{html.escape(str(case['setting']))}</td></tr>
    <tr><th>Allowed Claim</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>First Test</th><td>{html.escape(str(derivation['test']))}</td></tr>
    <tr><th>Stop Point</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
  </tbody>
</table>
<h3>Mental Model Pass Test</h3>
<p>The mental model passes only if the learner can redraw this route from memory as need, evidence, missing answer, move, shape, field reason, concrete picture, allowed claim, first test, and stop point.</p>
"""
...
def topic_application_claim_ladder_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    applications = topic_plain_applications(topic, derivation)
    rows = []
    for row in applications:
        rows.append(
            f"""
<tr>
  <td>{html.escape(str(row['field']))}</td>
  <td>{html.escape(str(row['use']))}</td>
  <td>{html.escape(str(derivation['observed']))}</td>
  <td>{html.escape(str(derivation['hidden']))}</td>
  <td>{html.escape(str(derivation['move']))}</td>
  <td>{html.escape(str(row['check']))}</td>
</tr>
"""
        )
    return f"""
<h2>Application Claim Ladder</h2>
<p>Each application has to climb the same ladder before it becomes believable. Name the field use, name the evidence, name the missing answer, name the plain move, and end with the first check that can reject the claim.</p>
<table>
  <thead><tr><th>Application Field</th><th>Plain Use</th><th>Evidence To Start From</th><th>Hidden Answer Needed</th><th>First-Principles Move</th><th>Changed Case That Tests It</th></tr></thead>
  <tbody>{''.join(rows)}</tbody>
</table>
...
def topic_field_decision_story_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    ...
    case = topic_case_walkthrough(topic, derivation)
    story = (
        f"A field decision starts before the topic name. "
        f"Someone needs to decide because {str(topic['why_it_matters'])}. "
        f"They can point to {str(derivation['observed'])}, but still need {str(derivation['hidden'])}. "
        f"{title} helps only if the move is clear: {str(derivation['move'])}. "
        f"The decision should stay narrow until this changed case is checked: {str(derivation['test'])}."
    )
    return f"""
<h2>Everyday Field Decision Story</h2>
<p>{html.escape(story)}</p>
<table>
  <tbody>
    <tr><th>Decision Being Made</th><td>{html.escape(str(topic['why_it_matters']))}</td></tr>
    <tr><th>Evidence On The Table</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Missing Answer For The Decision</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Plain Move That Connects Them</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape Issue Inside The Decision</th><td>{html.escape(str(shape['use']))} Check: {html.escape(str(shape['check']))}</td></tr>
    <tr><th>Engineering Decision Use</th><td>{html.escape(str(engineering['use']))}</td></tr>
    <tr><th>Materials Or Biology Decision Use</th><td>{html.escape(str(materials['use']))}</td></tr>
    <tr><th>Climate Or Field Decision Use</th><td>{html.escape(str(fields['use']))}</td></tr>
    <tr><th>Concrete Decision Picture</th><td>{html.escape(str(case['setting']))}</td></tr>
    <tr><th>Decision Claim Allowed</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>Decision Must Stop When</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
  </tbody>
</table>
<h3>Decision Story Pass Test</h3>
<p>The story passes only if the learner can name the decision, evidence, missing answer, plain move, shape issue, field uses, allowed claim, and stop point without using the topic name as the reason.</p>
"""
...
def topic_know_still_test_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    return f"""
<h2>What I Know And What I Still Test</h2>
<p>This is the final self-check before leaving the topic. Understanding means the reader can say what is known in plain words. Trust still needs tests, because a clear explanation can still be too broad.</p>
<table>
  <tbody>
    <tr><th>What I Know</th><td>{html.escape(str(topic['common_problem']))}</td></tr>
    <tr><th>Evidence I Can Point To</th><td>{html.escape(str(derivation['observed']))}</td></tr>
    <tr><th>Answer I Still Need</th><td>{html.escape(str(derivation['hidden']))}</td></tr>
    <tr><th>Move That Tries To Get There</th><td>{html.escape(str(derivation['move']))}</td></tr>
    <tr><th>Shape Or Topology I Must Not Lose</th><td>{html.escape(str(shape['use']))}</td></tr>
    <tr><th>Claim I Can Say Carefully</th><td>{html.escape(str(derivation['meaning']))}</td></tr>
    <tr><th>Test Still Needed</th><td>{html.escape(str(derivation['test']))}</td></tr>
    <tr><th>Stop Or Narrow When</th><td>{html.escape(str(topic['failure_boundary']))}</td></tr>
  </tbody>
</table>
...
def topic_slow_importance_essay_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    ...
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Why This Matters Slowly</h2>
<p>Do not start by asking what {html.escape(title)} is called. Start by asking what would be hard without it. In plain words, the hard part is this: {html.escape(str(topic['common_problem']))}. A person may have measurements, solved examples, a known equation, a shape, a field, a graph, a material sample, or a past experiment. That evidence is useful, but it is not yet the answer. The missing answer is {html.escape(str(derivation['hidden']))}. This topic matters because it gives a disciplined way to move from the evidence to that missing answer.</p>
<p>The move is {html.escape(str(derivation['move']))}. That sentence is the center of the idea. It says what work the topic is doing before any notation appears. It also limits the claim. The topic is allowed to say this: {html.escape(str(derivation['meaning']))}. It is not allowed to say that every future case is handled, or that a neat curve, small error, or good-looking field is enough. A scientific claim has to carry its own stopping rule.</p>
<p>Topology and shape matter here because scientific evidence often has form before it has numbers. {html.escape(str(shape['use']))} The reason is simple: {html.escape(str(shape['why']))} If a model loses the relation that carries the quantity, the answer can look tidy and still be wrong. The first shape check is: {html.escape(str(shape['check']))}</p>
<p>In engineering design, the same idea becomes a decision about a part, device, load, safety limit, or operating condition. {html.escape(str(engineering['use']))} Why it matters: {html.escape(str(engineering['why']))} The first engineering check is: {html.escape(str(engineering['check']))}</p>
...
def topic_across_fields_long_walk_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    ...
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Across Fields Long Walk</h2>
<p>Use this section to slow down and ask why the same idea matters after the example changes. The starting shortage is still this: {html.escape(str(topic['common_problem']))}. The evidence in hand is still {html.escape(str(derivation['observed']))}. The missing answer is still {html.escape(str(derivation['hidden']))}. {html.escape(title)} matters only if the move, {html.escape(str(derivation['move']))}, still carries the part of the evidence that the new field needs.</p>
<h3>One Plain Need Across Fields</h3>
<p>The plain need is not a method name. It is a person needing an answer they cannot see directly. In shape work, the person asks whether the answer changes when connection, boundary, hole, distance, mesh, or surface changes. In engineering, the person asks whether a design choice is still safe or useful. In materials, chemistry, or biology, the person asks whether a property, sample, molecule, tissue, or reaction still behaves the same way. In climate, fluids, or field work, the person asks whether the answer still holds across space, time, sources, and boundaries.</p>
<h3>Topology And Shape Walk</h3>
<p>For topology and shape, the plain use is: {html.escape(str(shape['use']))} This matters because {html.escape(str(shape['why']))} The first honest check is: {html.escape(str(shape['check']))} The learner should say what relation is being kept before they trust the answer. If the relation changes and the claim does not change, the explanation is probably too broad.</p>
<h3>Engineering Walk</h3>
<p>For engineering, the plain use is: {html.escape(str(engineering['use']))} This matters because {html.escape(str(engineering['why']))} The first honest check is: {html.escape(str(engineering['check']))} The learner should name the design quantity, the load, the boundary, the operating condition, or the stop point that would make the answer matter to a real build.</p>
<h3>Materials Chemistry Biology Walk</h3>
<p>For materials, chemistry, and biology, the plain use is: {html.escape(str(materials['use']))} This matters because {html.escape(str(materials['why']))} The first honest check is: {html.escape(str(materials['check']))} The learner should name the sample, molecule, structure, tissue, measurement, or condition that the claim must survive.</p>
<h3>Climate Fluids Fields Walk</h3>
...
def topic_why_not_simpler_answer_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    engineering = next(row for row in applications if row["field"] == "Engineering design")
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    return f"""
<h2>Why Not The Simpler Answer</h2>
<p>A beginner should ask the blunt question: why not use the simpler answer? Why not copy the nearest old case, fit a curve, read the visible pattern, or trust the first result that looks close? That question is useful. It keeps {html.escape(title)} from sounding like a label that appears for no reason.</p>
<h3>The Tempting Shortcut</h3>
<p>The tempting shortcut is to stop at the evidence we already have: {html.escape(str(derivation['observed']))}. That can feel reasonable because visible evidence is concrete. It is something a person can point to, plot, measure, or compare. But it is not the whole job.</p>
<h3>What The Shortcut Misses</h3>
<p>The shortcut misses this hidden answer: {html.escape(str(derivation['hidden']))}. That missing answer is why the topic enters. The first-principles move is to {html.escape(str(derivation['move']))}. Without that move, the learner may have a familiar-looking result but not the answer the scientific job needs.</p>
<h3>Shape The Shortcut Can Lose</h3>
<p>The shortcut can also lose the shape or topology relation that carries the answer. For this topic, the shape issue is: {html.escape(str(shape['use']))} The first shape check is: {html.escape(str(shape['check']))}</p>
<h3>Work The Shortcut Can Mislead</h3>
<p>In engineering, the shortcut can mislead because {html.escape(str(engineering['why']))} In field work, the shortcut can mislead because {html.escape(str(fields['why']))} The problem is not that simple answers are always bad. The problem is that a simple answer must still carry the evidence needed for the decision or quantity at stake.</p>
<h3>When The Simpler Answer Is Enough</h3>
<p>The simpler answer is enough only when it can name the evidence, name the missing answer, preserve the shape or field relation, and survive the changed case. If it can do all of that, the topic may only be a careful way to say the same thing. If it cannot, the shortcut is not an explanation yet.</p>
<h3>Simpler Answer Pass Test</h3>
<p>The pass test is this: try the simpler answer first, then change one thing using this test: {html.escape(str(derivation['test']))}. If the answer breaks, the learner must explain why {html.escape(title)} is needed and limit the final claim to this: {html.escape(str(derivation['meaning']))}</p>
"""


def topic_self_diagnosis_check_html(topic: dict[str, object], derivation: dict[str, object]) -> str:
    title = str(topic["title"])
    applications = topic_plain_applications(topic, derivation)
    shape = next(row for row in applications if row["field"] == "Topology and shape")
    fields = next(row for row in applications if row["field"] == "Climate, fluids, and fields")
    rows = [
        (
            "I only named the topic",
            f"Start with the shortage: {str(topic['common_problem'])}",
        ),
        (
            "I did not name the evidence",
            f"Add what is seen: {str(derivation['observed'])}",
        ),
        (
            "I did not name the missing answer",
            f"Add what is needed: {str(derivation['hidden'])}",
        ),
        (
            "I skipped the move",
            f"Add the plain move: {str(derivation['move'])}",
        ),
        (
            "I ignored shape or topology",
            f"Add the shape stake: {str(shape['use'])}",
        ),
        (
            "I ignored field use",
            f"Add the field stake: {str(fields['use'])}",
        ),
        (
            "I made the claim too broad",
            f"Limit the claim to: {str(derivation['meaning'])}",
        ),
        (
            "I ended without a test",
            f"End with the changed case: {str(derivation['test'])}",
        ),
    ]
    body_rows = "".join(
        f"""
<tr>
  <td>{html.escape(symptom)}</td>
  <td>{html.escape(repair)}</td>
</tr>
"""
        for symptom, repair in rows
    )
    return f"""
<h2>Self-Diagnosis Check</h2>
<p>Use this check after trying to explain {html.escape(title)} from memory. Do not ask whether the answer sounds smart. Ask which sentence is missing. A plain explanation is strong only when each missing sentence has been repaired.</p>
<table>
  <thead><tr><th>Weak Sentence Symptom</th><th>Repair Sentence To Add</th></tr></thead>
  <tbody>{body_rows}</tbody>
</table>
<h3>Self-Diagnosis Pass Test</h3>
<p>The self-diagnosis passes only if the learner can find the missing sentence, add it in everyday words, and then say the whole chain again: shortage, evidence, missing answer, move, shape stake, field stake, careful claim, and changed-case test.</p>
"""
...
"""
    ...
    proof_boundary = proof_boundary_html(dict(packet["proof_boundary"]))
    body = f"""
    ...
    for packet in data["concept_evidence_packets"]:
        ...
        lines.extend(
            [
                ...
                f"- Transcript can support: {boundary['transcript_can_support']}",
                f"- Transcript cannot support: {boundary['transcript_cannot_support']}",
                f"- Stronger validation needed: {boundary['stronger_validation_needed']}",
                f"- First overclaim to reject: {boundary['first_overclaim_to_reject']}",
                f"- Reviewer action: {boundary['reviewer_action']}",
...
def validate(data: dict[str, object] | None = None) -> None:
    ...
    for slug in TOPIC_DEEP_DIVES:
        ...
        required_topic_terms = (
            ...
            "Across Fields Long Walk",
            "One Plain Need Across Fields",
            "Topology And Shape Walk",
            "Engineering Walk",
            ...
            "Why Not The Simpler Answer",
            "The Tempting Shortcut",
            "What The Shortcut Misses",
            "Shape The Shortcut Can Lose",
            "Work The Shortcut Can Mislead",
            "When The Simpler Answer Is Enough",
            "Simpler Answer Pass Test",
            "Self-Diagnosis Check",
            "Weak Sentence Symptom",
            "Repair Sentence To Add",
            "I only named the topic",
            "I did not name the evidence",
            "I did not name the missing answer",
            "I skipped the move",
    ...
    for packet in packets:
        ...
        for required in ("Source Strength Audit", "Reviewed Source Anchors", "Broad Transcript Mentions", "Minimum Review Action", "Stronger Proof Needed", "Claim Boundary Review", "Overclaim To Avoid", "First Rejection Test", "Transcript Evidence vs Scientific Proof", "Transcript Can Support", "Transcript Cannot Support", "Stronger Validation Needed", "First Overclaim To Reject", "Reviewer Action", "Transcript Support", "What This Evidence Does Not Prove", "Review Links"):
        ...
        for field in ("transcript_can_support", "transcript_cannot_support", "stronger_validation_needed", "first_overclaim_to_reject", "reviewer_action"):
            if not proof_boundary.get(field):
                raise SystemExit(f"concept evidence packet missing proof boundary {field}: {packet['title']}")
        strength = packet.get("source_strength") or {}
